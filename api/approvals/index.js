...
async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    const { request_id, decision, comment, salary_amount } = await readBody(req);
    if (!request_id) return res.status(400).json({ error: 'request_id is required' });
    if (!['Approved', 'Rejected'].includes(decision)) {
      return res.status(400).json({ error: 'decision must be Approved or Rejected' });
    }

    const request = await get('SELECT * FROM certificate_requests WHERE request_id = ?', [request_id]);
